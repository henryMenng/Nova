...
{
    public partial class NovaFileProvider : PhysicalFileProvider, INovaFileProvider
    {
        #region Ctor
        ...
        public NovaFileProvider(IHostEnvironment hostEnvironment)
            : base(!string.IsNullOrEmpty(hostEnvironment.ContentRootPath) && File.Exists(hostEnvironment.ContentRootPath)
            ? Path.GetDirectoryName(hostEnvironment.ContentRootPath)!
            : hostEnvironment.ContentRootPath ?? throw new ArgumentNullException(nameof(hostEnvironment.ContentRootPath)))
        {
            AppRootPath = !string.IsNullOrEmpty(hostEnvironment.ContentRootPath) && File.Exists(hostEnvironment.ContentRootPath)
                ? Path.GetDirectoryName(hostEnvironment.ContentRootPath)!
                : hostEnvironment.ContentRootPath ?? throw new ArgumentNullException(nameof(hostEnvironment.ContentRootPath));
        }
        ...
        public virtual string GetAbsolutePath(params string[] paths)
        {
            ...
            if (paths.Any() && !paths[0].Contains(AppRootPath, StringComparison.InvariantCulture))
                allPaths.Add(AppRootPath);
        ...
        public virtual string GetVirtualPath(string path)
        {
            ...
            if (path != null)
            {
                path = path.Replace(AppRootPath, string.Empty)
                           .Replace('\\', '/')
                           .Trim('/')
                           .TrimStart('~', '/');
            }
